        ...
        async with aiohttp.ClientSession() as session:
            ...
            if download == True and response.status == 200:
                ...
                async for part in reader:
                    await save_file_from_response(part)
...
async def save_file_from_response(response, default_filename='downloaded_file'):

    filename = response.headers.get('Content-Disposition', '').split('filename=')[1].strip('"') if 'filename=' in response.headers.get('Content-Disposition', '') else default_filename
    ...
    async with aiofiles.open(save_path, 'wb') as f:
        while True:
            chunk = await response.read_chunk()
            if not chunk:
                break
            await f.write(chunk)
